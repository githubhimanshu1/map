...
var mapUtility = {
  init: function init(ref, data) {
    var region =
      arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "us";
    mapUtility.regionloader(ref, mapMetaData[region]["data"], region);
  },
  regionloader: function regionloader(ref, data, region) {
    console.log(mapMetaData[region]["mapname"]); // Create the chart
      ...
      series: [
        {
          ...
          dataLabels: {
            enabled: true,
            format:
              '<div class="circles" onmouseover="mapUtility.tooltip(event,{point.value})">{point.value}</div>',
            useHTML: true
          }
        }
      ]
    });
    console.log("#".concat(ref));
    jQuery("#".concat(ref)).hide().fadeIn(800);
  },
  tooltip: function tooltip(event, value) {
